...
class DataSenderApp:
    def __init__(self, root):
        ...
        self.api_url = tk.StringVar(value="http://127.0.0.1:8000/dadosColetados/")
    ...
    def create_widgets(self):
        frame_server = ttk.LabelFrame(self.root, text="Configuração da API")
        frame_server.grid(row=0, column=0, padx=10, pady=10, sticky="ew")

        ttk.Label(frame_server, text="URL da API:").grid(row=0, column=0, padx=5, pady=5, sticky="e")
        self.api_url_entry = ttk.Entry(frame_server, textvariable=self.api_url, width=50)
        self.api_url_entry.grid(row=0, column=1, padx=5, pady=5)

        self.start_button = ttk.Button(frame_server, text="Iniciar Envio", command=self.start_sending)
        self.start_button.grid(row=1, column=0, columnspan=2, pady=5)

        self.stop_button = ttk.Button(frame_server, text="Parar Envio", command=self.stop_sending, state="disabled")
        ...
        frame_data.grid(row=2, column=0, padx=10, pady=10, sticky="ew")
        ...
        while self.dados_restantes and self.enviar_evento.is_set():
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.data_list.insert(tk.END, f"Erro: {response.status_code} - {response.text}")
    ...
    def validate_inputs(self):
        ...
        if not self.api_url.get():
            messagebox.showerror("Erro", "A URL da API não pode estar vazia.")
            return False
        return True
